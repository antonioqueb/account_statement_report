...
class AddDocument(models.Model):
    ...
    subtotal = fields.Float(digits=(24, 6), aggregator=None)
    discount = fields.Float(digits=(24, 6), aggregator=None)
    base = fields.Float(digits=(24, 6), aggregator=None)
    net_base = fields.Float(digits=(24, 6), aggregator=None, string='Base neta documental I − E')
    total = fields.Float(digits=(24, 6), aggregator=None)
    mxn_available = fields.Boolean(index=True)
    base_mxn = fields.Float(digits=(24, 6), aggregator=None)
    total_mxn = fields.Float(digits=(24, 6), aggregator=None)
    conversion_source = fields.Char()
    vat = fields.Float(digits=(24, 6), aggregator=None)
    withheld = fields.Float(digits=(24, 6), aggregator=None)
...
class AddConcept(models.Model):
    ...
    def _compute_tax_filter(self):
        for concept in self:
            concept.tax_filter = ', '.join(sorted(code for code in set(concept.tax_ids.mapped('tax')) if code))
